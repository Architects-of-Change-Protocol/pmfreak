...
const config = createAocPMFreakReadOnlyConnectorConfig({ sourceKind: "supabase" });
...
test("unsupported real source adapter fails safely for every method and every real source kind", async () => {
  ...
  for (const kind of kinds) {
    const source = createUnsupportedAocPMFreakRealReadOnlySource(kind);
    ...
    for (const method of methods) {
      await assert.rejects(() => source[method](config), (error: unknown) => {
        const connectorError = error as { safe?: boolean; code?: string };
        assert.equal(connectorError.safe, true);
        assert.equal(connectorError.code, "unsupported_source_kind");
        return true;
      });
    }
  }
...
test("unsupported real source adapter never requires env vars or credentials", () => {
  assert.doesNotThrow(() => createUnsupportedAocPMFreakRealReadOnlySource("database"));
});

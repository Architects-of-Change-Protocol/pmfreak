...
import { makeCookieJar, makeFakeGoTrueFactory, mockModuleOptions, resolveMockTarget } from "./release-gate-01-fake-supabase.mjs";
...
test("PRE-FIX PATTERN: assertRuntimeAuthContinuity() followed by a second, independent getAuthUser() call loses the session and requireAuthUser() redirects to /login, despite a genuinely authenticated first call", async (t) => {
  ...
  t.mock.module(resolveMockTarget("next/headers"), mockModuleOptions({
    cookies: async () => jar,
    headers: async () => ({ get: (name) => (name === "x-pathname" ? "/command-center" : null) }),
  }));
  t.mock.module(resolveMockTarget("@supabase/ssr"), mockModuleOptions({ createServerClient: makeFakeGoTrueFactory() }));

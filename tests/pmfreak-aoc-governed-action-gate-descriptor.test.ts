...
import { createPMFreakAocGovernedActionGateDescriptor, PMFREAK_AOC_GOVERNED_ACTION_GATE_ID } from "../src/features/pmfreak-integrations/aoc-governance-request-client";

test("descriptor has the correct feature ID", () => {
  const descriptor = createPMFreakAocGovernedActionGateDescriptor();
  assert.equal(descriptor.featureId, PMFREAK_AOC_GOVERNED_ACTION_GATE_ID);
  assert.equal(descriptor.featureId, "pmfreak.integration.aoc.governed_action_gate.v1");
});
...
test("descriptor states gateOnly = true", () => {
  const descriptor = createPMFreakAocGovernedActionGateDescriptor();
  assert.equal(descriptor.gateOnly, true);
});
...
test("descriptor lists capabilities and forbidden operations", () => {
  const descriptor = createPMFreakAocGovernedActionGateDescriptor();
  assert.ok(descriptor.capabilities.includes("evaluate_governance_response"));
  assert.ok(descriptor.capabilities.includes("evaluate_action_attempt"));

...
import { redactPMFreakAocSurfaceValue } from "../src/features/pmfreak-integrations/aoc-read-only-surface";
...
test("redaction mode none leaves values as-is and does not mutate input", () => {
  ...
  const result = redactPMFreakAocSurfaceValue(input, "none") as ReturnType<typeof sample>;
...
test("redaction mode safe_demo removes obvious email/token/secret/authorization/connection-string values", () => {
  ...
  const result = redactPMFreakAocSurfaceValue(input, "safe_demo") as ReturnType<typeof sample>;
...
test("redaction mode strict removes metadata and redacts sourceUrl in addition to safe_demo behavior", () => {
  ...
  const result = redactPMFreakAocSurfaceValue(input, "strict") as ReturnType<typeof sample>;

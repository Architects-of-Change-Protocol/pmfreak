...
import { demoPMFreakAocEvidenceRequirementHandoffMissingCustomerAcceptance } from "../src/features/pmfreak-integrations/aoc-governance-request-client";
...
test("evidence requirement handoff module contains no non-deterministic or network/AI-calling APIs", () => {
  const files = listHandoffFiles();
  assert.ok(files.length > 0, "expected to find evidence requirement handoff source files");

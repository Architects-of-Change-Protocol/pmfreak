# P2-03 — Raw Input and Normalized Event Foundation

## Prompt Metadata

- **Prompt ID:** P2-03
- **Work Package:** WP2
- **Title:** Raw Input and Normalized Event Foundation
...
A controlled manual demo submission becomes an immutable Raw Input and versioned Normalized Event, visibly labelled fixture/demo, without creating Evidence directly.

## Current State and Evidence

- **Reusable:** src/lib/operational-flow/; src/app/api/operational-flow/; supabase/migrations/; tests/operational-flow-contract.test.mjs; scripts/check-operational-flow-db.mjs.
...
- **Missing for this increment:** Add source registry semantics, content-addressed raw capture, normalized event schema/version, idempotency and authorized retrieval; use additive persistence and RLS.
- **Candidate adapters:** prefer existing services/ports in the listed areas; inventory consumers before adding a parallel model.

## Scope

Add source registry semantics, content-addressed raw capture, normalized event schema/version, idempotency and authorized retrieval; use additive persistence and RLS. End when this outcome is behaviorally tested and independently reviewable; do not absorb downstream prompts.
...
Required state: G1. Every dependency must be `VERIFIED`; a contract-based parallel start is allowed only where metadata says so. Use an isolated development database for migrations/runtime tests. D1–D7 are ratified. Any fixture must say `DEMO / FIXTURE`, conform to the verified contract, and expire when P2-04, P2-13, P2-17 becomes verified. Inspect migration ordering and overlapping working-tree changes before editing.
...
- **Data:** migration `yes additive`; no destructive operation or silent dual-write. If temporary compatibility read/write is necessary, document owner, expiry prompt and reconciliation.
...
Expected areas: `src/lib/operational-flow/; src/app/api/operational-flow/; supabase/migrations/; tests/operational-flow-contract.test.mjs; scripts/check-operational-flow-db.mjs`. Tests and narrowly scoped docs may change. Adjust paths only when better repository evidence is found and justify every deviation. Migration files, if needed, must be new and forward-only.

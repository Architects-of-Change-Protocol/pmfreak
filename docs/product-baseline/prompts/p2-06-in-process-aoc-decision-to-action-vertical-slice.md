# P2-06 — In-Process AOC Decision-to-Action Vertical Slice

## Prompt Metadata

- **Prompt ID:** P2-06
- **Work Package:** WP3
- **Title:** In-Process AOC Decision-to-Action Vertical Slice
- **Phase:** Founder Invite
- **Primary Track:** Track B/A/C
- **Parallelizable:** no
- **Depends On:** P2-04 and P2-05 VERIFIED
- **Unlocks:** P2-07, P2-11, G2
- **Risk Level:** high
...
An authorized PM requests a separate Action from a Decision and sees AOC-E in-process allow, deny, revoke or unavailable without Task side effects.

## Current State and Evidence

- **Reusable:** src/lib/operational-flow/; src/aoc/runtime-consumer/; src/app/api/operational-flow/; src/components/pmfreak/intelligence-inbox/; supabase/migrations/; tests/.
...
- **Missing for this increment:** Connect operational Decision to AOC-E policy/grant via adapter, persist Action/request outcome, expose UI state and correlated audit; remote mode stays advisory/writeback false.
- **Candidate adapters:** prefer existing services/ports in the listed areas; inventory consumers before adding a parallel model.

## Scope

Connect operational Decision to AOC-E policy/grant via adapter, persist Action/request outcome, expose UI state and correlated audit; remote mode stays advisory/writeback false. End when this outcome is behaviorally tested and independently reviewable; do not absorb downstream prompts.
...
Required state: P2-04 and P2-05 VERIFIED. Every dependency must be `VERIFIED`; a contract-based parallel start is allowed only where metadata says so. Use an isolated development database for migrations/runtime tests. D1–D7 are ratified. Any fixture must say `DEMO / FIXTURE`, conform to the verified contract, and expire when P2-07, P2-11, G2 becomes verified. Inspect migration ordering and overlapping working-tree changes before editing.
...
```bash
npx tsx --test tests/operational-flow-contract.test.mjs tests/pmfreak-aoc-action-attempt.test.ts tests/local-authority-bypass-hardening.test.mjs
npm run typecheck
npm run lint
git diff --check
```

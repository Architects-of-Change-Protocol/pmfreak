# P2-05 — Material Action and Governance Contract

## Prompt Metadata

- **Prompt ID:** P2-05
- **Work Package:** WP3
- **Title:** Material Action and Governance Contract
- **Phase:** Founder Invite
- **Primary Track:** Track B
- **Parallelizable:** yes after G1
- **Depends On:** G1
- **Unlocks:** P2-06
- **Risk Level:** high
...
A PM can see whether a proposed Action is ordinary business work or material and which AOC policy/grant is required, without creating a Task.

## Current State and Evidence

- **Reusable:** src/aoc/runtime-consumer/; src/aoc/enterprise/runtime/; src/features/pmfreak-integrations/aoc-governance-request-client/; src/lib/operational-flow/; tests/pmfreak-aoc-*.test.ts.
...
- **Missing for this increment:** Define Decision→Action request/result, material taxonomy D5, obligations/grant references and states; preserve business authority distinction and remote writeback false.
- **Candidate adapters:** prefer existing services/ports in the listed areas; inventory consumers before adding a parallel model.

## Scope

Define Decision→Action request/result, material taxonomy D5, obligations/grant references and states; preserve business authority distinction and remote writeback false. End when this outcome is behaviorally tested and independently reviewable; do not absorb downstream prompts.
...
Required state: G1. Every dependency must be `VERIFIED`; a contract-based parallel start is allowed only where metadata says so. Use an isolated development database for migrations/runtime tests. D1–D7 are ratified. Any fixture must say `DEMO / FIXTURE`, conform to the verified contract, and expire when P2-06 becomes verified. Inspect migration ordering and overlapping working-tree changes before editing.
...
```bash
npx tsx --test tests/pmfreak-aoc-governed-action-gate-evaluator.test.ts tests/local-authority-bypass-hardening.test.mjs
npm run typecheck
npm run lint
git diff --check
```
...
Expected areas: `src/aoc/runtime-consumer/; src/aoc/enterprise/runtime/; src/features/pmfreak-integrations/aoc-governance-request-client/; src/lib/operational-flow/; tests/pmfreak-aoc-*.test.ts`. Tests and narrowly scoped docs may change. Adjust paths only when better repository evidence is found and justify every deviation. Migration files, if needed, must be new and forward-only.

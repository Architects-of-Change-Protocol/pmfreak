...
- **Prompt ID:** P2-08
- **Work Package:** WP4
- **Title:** Idempotent Internal Dispatch and Execution Experience
- **Phase:** Founder Invite
- **Primary Track:** Track A/C
- **Parallelizable:** no
- **Depends On:** P2-07 VERIFIED
...
Retrying an authorized Action creates exactly one internal Task; the PM sees queued, running, blocked, failed and completed execution without Outcome auto-close.

## Current State and Evidence

- **Reusable:** src/app/api/execution-tasks/; src/components/pmfreak/tasks/; src/components/pmfreak/execution/; src/lib/task-drafts/; tests/.
...
- **Missing for this increment:** Implement transactional/idempotent dispatch, reconciliation, activity/audit and project UI states; provider abstraction remains internal and external providers out of scope.
- **Candidate adapters:** prefer existing services/ports in the listed areas; inventory consumers before adding a parallel model.

## Scope

Implement transactional/idempotent dispatch, reconciliation, activity/audit and project UI states; provider abstraction remains internal and external providers out of scope. End when this outcome is behaviorally tested and independently reviewable; do not absorb downstream prompts.
...
Required state: P2-07 VERIFIED. Every dependency must be `VERIFIED`; a contract-based parallel start is allowed only where metadata says so. Use an isolated development database for migrations/runtime tests. D1–D7 are ratified. Any fixture must say `DEMO / FIXTURE`, conform to the verified contract, and expire when P2-09, P2-12, G2 becomes verified. Inspect migration ordering and overlapping working-tree changes before editing.
...
```bash
npx tsx --test tests/execution-tasks.test.mjs tests/execution-task-update-validation.test.ts tests/execution-task-write-authorization.test.mjs
npm run typecheck
npm run lint
git diff --check
```
...
Expected areas: `src/app/api/execution-tasks/; src/components/pmfreak/tasks/; src/components/pmfreak/execution/; src/lib/task-drafts/; tests/`. Tests and narrowly scoped docs may change. Adjust paths only when better repository evidence is found and justify every deviation. Migration files, if needed, must be new and forward-only.

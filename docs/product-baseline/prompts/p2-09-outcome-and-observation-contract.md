...
- **Prompt ID:** P2-09
- **Work Package:** WP5
- **Title:** Outcome and Observation Contract
- **Phase:** Founder Invite
- **Primary Track:** Track A
- **Parallelizable:** no
- **Depends On:** P2-08 VERIFIED
- **Unlocks:** P2-10
- **Risk Level:** high
- **Expected Review Size:** large
- **Status:** `NOT_STARTED`
- **Migration:** yes additive likely; forward-only, additive if used; destructive changes prohibited.
...
Completing a Task leaves an expected Outcome pending until an authorized observer records evidence-backed achieved, partial, failed, disputed or inconclusive observation.

## Current State and Evidence

- **Reusable:** src/lib/decision-outcome-engine/; src/lib/operational-decision-outcome/; src/app/api/agents/execution/outcomes/; src/lib/operational-flow/; supabase/migrations/; tests/decision-outcome-engine.test.mjs.
...
- **Missing for this increment:** Create/adapt canonical Outcome and Observation contracts, persistence, authority, freshness and learning-candidate event only; prohibit achievement inference.
- **Candidate adapters:** prefer existing services/ports in the listed areas; inventory consumers before adding a parallel model.

## Scope

Create/adapt canonical Outcome and Observation contracts, persistence, authority, freshness and learning-candidate event only; prohibit achievement inference. End when this outcome is behaviorally tested and independently reviewable; do not absorb downstream prompts.
...
Required state: P2-08 VERIFIED. Every dependency must be `VERIFIED`; a contract-based parallel start is allowed only where metadata says so. Use an isolated development database for migrations/runtime tests. D1–D7 are ratified. Any fixture must say `DEMO / FIXTURE`, conform to the verified contract, and expire when P2-10 becomes verified. Inspect migration ordering and overlapping working-tree changes before editing.
...
- **Data:** migration `yes additive likely`; no destructive operation or silent dual-write. If temporary compatibility read/write is necessary, document owner, expiry prompt and reconciliation.
...
```bash
npx tsx --test tests/decision-outcome-engine.test.mjs tests/agent-controlled-execution-result-reconciliation-human-outcome-review.test.mjs
npm run typecheck
npm run lint
git diff --check
```
...
Expected areas: `src/lib/decision-outcome-engine/; src/lib/operational-decision-outcome/; src/app/api/agents/execution/outcomes/; src/lib/operational-flow/; supabase/migrations/; tests/decision-outcome-engine.test.mjs`. Tests and narrowly scoped docs may change. Adjust paths only when better repository evidence is found and justify every deviation. Migration files, if needed, must be new and forward-only.

...
- **Prompt ID:** P2-13
- **Work Package:** WP7
- **Title:** Founder Invite Seed and Isolated Environment Harness
- **Phase:** Founder Invite
- **Primary Track:** Track D
- **Parallelizable:** yes after P2-04
- **Depends On:** P2-04 VERIFIED; G1
- **Unlocks:** P2-14
- **Risk Level:** high
...
An operator can seed/reset an explicitly labelled deterministic Founder scenario in isolated infrastructure without secrets or cross-tenant residue.

## Current State and Evidence

- **Reusable:** scripts/seed-operational-flow-demo.mjs; scripts/check-operational-flow-db.mjs; .env.operational-flow.example; tests/; docs/.
...
- **Missing for this increment:** Extend seed/verifier for full spine, two tenants, deterministic IDs, cleanup, observability and documented limitations; never target production.
- **Candidate adapters:** prefer existing services/ports in the listed areas; inventory consumers before adding a parallel model.

## Scope

Extend seed/verifier for full spine, two tenants, deterministic IDs, cleanup, observability and documented limitations; never target production. End when this outcome is behaviorally tested and independently reviewable; do not absorb downstream prompts.
...
Required state: P2-04 VERIFIED; G1. Every dependency must be `VERIFIED`; a contract-based parallel start is allowed only where metadata says so. Use an isolated development database for migrations/runtime tests. D1–D7 are ratified. Any fixture must say `DEMO / FIXTURE`, conform to the verified contract, and expire when P2-14 becomes verified. Inspect migration ordering and overlapping working-tree changes before editing.
...
- **Data:** migration `possible additive only through prior migrations`; no destructive operation or silent dual-write. If temporary compatibility read/write is necessary, document owner, expiry prompt and reconciliation.
...
Expected areas: `scripts/seed-operational-flow-demo.mjs; scripts/check-operational-flow-db.mjs; .env.operational-flow.example; tests/; docs/`. Tests and narrowly scoped docs may change. Adjust paths only when better repository evidence is found and justify every deviation. Migration files, if needed, must be new and forward-only.

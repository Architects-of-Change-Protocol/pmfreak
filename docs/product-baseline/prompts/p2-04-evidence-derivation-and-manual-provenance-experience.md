# P2-04 — Evidence Derivation and Manual Provenance Experience

## Prompt Metadata

- **Prompt ID:** P2-04
- **Work Package:** WP2
- **Title:** Evidence Derivation and Manual Provenance Experience
- **Phase:** Founder Invite
- **Primary Track:** Track A/C
- **Parallelizable:** no
- **Depends On:** P2-03 VERIFIED
- **Unlocks:** P2-06, P2-11, G2
- **Risk Level:** high
- **Expected Review Size:** large
- **Status:** `VERIFIED`
- **Migration:** yes additive/backfill possible; forward-only, additive if used; destructive changes prohibited.
...
A PM submits explicitly labelled demo input and inspects Raw Input→Normalized Event→Evidence provenance, digest, classification and freshness before intelligence runs.

## Current State and Evidence

- **Reusable:** src/lib/operational-flow/operational-flow-service.ts; src/app/api/operational-flow/route.ts; src/components/pmfreak/intelligence-inbox/; supabase/migrations/; tests/.
...
- **Missing for this increment:** Derive Evidence only from normalized event; replace zero hash; add fact/inference/assumption state, correlation, duplicate handling, UI loading/error/stale states and future connector-neutral boundary.
- **Candidate adapters:** prefer existing services/ports in the listed areas; inventory consumers before adding a parallel model.

## Scope

Derive Evidence only from normalized event; replace zero hash; add fact/inference/assumption state, correlation, duplicate handling, UI loading/error/stale states and future connector-neutral boundary. End when this outcome is behaviorally tested and independently reviewable; do not absorb downstream prompts.
...
Required state: P2-03 VERIFIED. Every dependency must be `VERIFIED`; a contract-based parallel start is allowed only where metadata says so. Use an isolated development database for migrations/runtime tests. D1–D7 are ratified. Any fixture must say `DEMO / FIXTURE`, conform to the verified contract, and expire when P2-06, P2-11, G2 becomes verified. Inspect migration ordering and overlapping working-tree changes before editing.
...
- **Data:** migration `yes additive/backfill possible`; no destructive operation or silent dual-write. If temporary compatibility read/write is necessary, document owner, expiry prompt and reconciliation.
...
```bash
npx tsx --test tests/operational-flow-contract.test.mjs
npm run typecheck
npm run lint
git diff --check
```

# P2-01 — Canonical Domain Contract and Consumer Map

## Prompt Metadata

- **Prompt ID:** P2-01
- **Work Package:** WP1
- **Title:** Canonical Domain Contract and Consumer Map
...
- **Status:** `VERIFIED`
- **Migration:** none; forward-only, additive if used; destructive changes prohibited.
...
An authorized team can identify one canonical Recommendation→Decision→Action→Task→Outcome lineage while every legacy model remains explicitly classified and compatible.

## Current State and Evidence

- **Reusable:** src/lib/operational-flow/; src/lib/decision-governance/; src/lib/recommended-actions/; src/lib/task-drafts/; src/app/api/agents/execution/outcomes/; supabase/migrations/20260611000000_operational_evidence_decision_loop.sql.
...
- **Missing for this increment:** Inventory and export canonical IDs, lifecycle/state vocabulary, reference mappings and consumer classification; add behavioral contract tests; produce machine-readable consumer map.
- **Candidate adapters:** prefer existing services/ports in the listed areas; inventory consumers before adding a parallel model.

## Scope

Inventory and export canonical IDs, lifecycle/state vocabulary, reference mappings and consumer classification; add behavioral contract tests; produce machine-readable consumer map. End when this outcome is behaviorally tested and independently reviewable; do not absorb downstream prompts.
...
Required state: none. Every dependency must be `VERIFIED`; a contract-based parallel start is allowed only where metadata says so. Use an isolated development database for migrations/runtime tests. D1–D7 are ratified. Any fixture must say `DEMO / FIXTURE`, conform to the verified contract, and expire when P2-02, G1 becomes verified. Inspect migration ordering and overlapping working-tree changes before editing.
...
- **Data:** migration `none`; no destructive operation or silent dual-write. If temporary compatibility read/write is necessary, document owner, expiry prompt and reconciliation.
...
```bash
npx tsx --test tests/operational-flow-contract.test.mjs tests/evidence-linked-decisions.test.mjs
npm run typecheck
npm run lint
git diff --check
```
...
Expected areas: `src/lib/operational-flow/; src/lib/decision-governance/; src/lib/recommended-actions/; src/lib/task-drafts/; src/app/api/agents/execution/outcomes/; supabase/migrations/20260611000000_operational_evidence_decision_loop.sql`. Tests and narrowly scoped docs may change. Adjust paths only when better repository evidence is found and justify every deviation. Migration files, if needed, must be new and forward-only.

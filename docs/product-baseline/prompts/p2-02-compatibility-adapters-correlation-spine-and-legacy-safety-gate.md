# P2-02 — Compatibility Adapters, Correlation Spine and Legacy Safety Gate

## Prompt Metadata

- **Prompt ID:** P2-02
- **Work Package:** WP1
- **Title:** Compatibility Adapters, Correlation Spine and Legacy Safety Gate
- **Phase:** Founder Invite
- **Primary Track:** Track A
- **Parallelizable:** no
- **Depends On:** P2-01 VERIFIED
...
Canonical lineage references can traverse retained legacy records without changing their ownership or silently breaking consumers.

## Current State and Evidence

- **Reusable:** src/lib/operational-flow/; src/lib/recommended-actions/; src/lib/task-drafts/; src/lib/decision-governance/; src/lib/platform-events/; tests/.
...
- **Missing for this increment:** Implement additive adapters and correlation contracts, compatibility reads, event mapping and a gate that inventories consumers; no removal or destructive migration.
- **Candidate adapters:** prefer existing services/ports in the listed areas; inventory consumers before adding a parallel model.

## Scope

Implement additive adapters and correlation contracts, compatibility reads, event mapping and a gate that inventories consumers; no removal or destructive migration. End when this outcome is behaviorally tested and independently reviewable; do not absorb downstream prompts.
...
Required state: P2-01 VERIFIED. Every dependency must be `VERIFIED`; a contract-based parallel start is allowed only where metadata says so. Use an isolated development database for migrations/runtime tests. D1–D7 are ratified. Any fixture must say `DEMO / FIXTURE`, conform to the verified contract, and expire when G1, P2-03, P2-05 becomes verified. Inspect migration ordering and overlapping working-tree changes before editing.
...
- **Data:** migration `none/possible additive`; no destructive operation or silent dual-write. If temporary compatibility read/write is necessary, document owner, expiry prompt and reconciliation.
...
Expected areas: `src/lib/operational-flow/; src/lib/recommended-actions/; src/lib/task-drafts/; src/lib/decision-governance/; src/lib/platform-events/; tests/`. Tests and narrowly scoped docs may change. Adjust paths only when better repository evidence is found and justify every deviation. Migration files, if needed, must be new and forward-only.

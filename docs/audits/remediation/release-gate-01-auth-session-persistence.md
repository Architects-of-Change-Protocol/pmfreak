...
**Tests:** `tests/module-mocks/login-onboarding-resolution-failure-pre-fix.test.mjs` (documents the pre-fix throw against the real, unmodified dependency chain) and `login-onboarding-resolution-failure-fixed.test.mjs` (real, unmodified `POST` export of `route.ts`, asserting the redirect and cookie both survive). Verified: the fixed-behavior test fails with the exact uncaught exception when run against the pre-fix `route.ts` (confirmed via `git stash`, equivalent to `origin/main`'s version of this untouched-until-now file), and passes after the fix, on both Node 22.23.1 and Node 24.18.0.

**Scope note:** `src/app/signup/actions.ts` (the Server Action backing `/signup`, currently unused by `/login`'s own page but a live entry point) has the identical unguarded-`resolveOnboardingState`/`resolveCanonicalWorkspace`-after-cookie-write shape. Not fixed here — out of scope for this login-triggered UAT finding — and flagged in §15 as residual debt for a follow-up.
...
5. **`src/app/signup/actions.ts` has the same unguarded-cookie-write-then-DB-call shape §13a fixed in `/api/login/route.ts`** — `signUp()` writes the session cookie, then an un-guarded `resolveOnboardingState(authUser, (await resolveCanonicalWorkspace(authUser.id)).workspaceId)` runs before the final `redirect(...)`. Not fixed in this PR (out of scope — discovered via a login-triggered UAT finding, not exercised by any reproduction here); a signup-time DB hiccup could destroy a freshly-created session the same way. Flagged for a follow-up fix using the identical try/catch pattern.

...
**Cross-Node-version compatibility fix (discovered via a real CI failure on this PR, not anticipated in advance):** `mock.module()`'s exports-object option was renamed between Node majors — Node 22.x (through at least 22.22.2, this repo's baseline local/CI version at the time) only recognizes `namedExports`; a newer Node major (this repo's CI runner silently upgrades a pinned `node-version: '20'` to the runner's current default — 24.x at time of writing) recognizes `exports` and only *deprecates* `namedExports` (works, with a warning) — except passing both keys together throws `ERR_INVALID_ARG_VALUE`. `tests/module-mocks/release-gate-01-fake-supabase.mjs` exports a small `mockModuleOptions(exportsObj)` helper that picks the correct key from `process.versions.node` and returns the options object — it deliberately does *not* call `t.mock.module()` itself, so each test file still calls `t.mock.module(specifier, mockModuleOptions(...))` directly. That mattered in practice: an earlier version of this helper called `t.mock.module()` from inside itself, and in CI (not reproducible locally) Node's specifier resolution used that helper file's own location as the import context, trying to resolve `"next/headers"` as `tests/module-mocks/next/headers` and failing with `ERR_MODULE_NOT_FOUND`. Keeping the actual `t.mock.module()` call site in the test file itself avoids relying on exactly which module is allowed to be the resolution context.

...
The Knowledge Center (`03-canonical-information-architecture.md` §5.9, `03-screen-catalog.md`) has two distinct views — a Candidate Patterns view and a Ratified Patterns view — never merged into one undifferentiated "Patterns" list (`03-screen-catalog.md`'s binding rule for this screen). The Ratified Patterns view shows only knowledge that has passed the six-part elevation gate — evidence, confidence, review, lineage, applicability, ratification (ADR-PMF-029, ADR-PMF-040) — always with its provenance visible via the Provenance panel (`GetKnowledgeLineage`). The Candidate Patterns view is visible only to actors holding ratification authority (the same authority the Ratify Pattern modal requires, `03-screen-catalog.md`) — never to a general Knowledge Center viewer — and is never styled, badged, or interacted with as though it carried a Ratified record's authority; a reviewer moving between the two views always sees an unmistakable status change, not a subtle badge easily missed.

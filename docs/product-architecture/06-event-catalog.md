...
Purpose: give every event already catalogued in `04-command-query-event-catalog.md` §7 (37 events, counted directly from that table's rows — verified against the source table row by row) its API-facing classification — Domain, Integration, or Notification-triggering — and define the webhook strategy by which the Integration Event subset reaches external subscribers. This document does not add, rename, or redefine a single event; every event listed is taken verbatim from PR4, published via the transactional outbox PR5 already defined (ADR-PMF-037, PR5 §20).

**Category definitions (kept distinct per the task's charge, never conflated):**
- **Domain Event** — internal fact of the domain, published for any consumer within the platform (other bounded contexts, projections, workflows). All 37 events below are Domain Events; every Domain Event is a candidate outbox record. Fourteen of the 37 are additionally Integration Event candidates (§8 of the source catalog); the remaining twenty-three are Domain-Event-only until a second-context consumer is designed.
...
- **Notification Event** — a Domain Event consumed by the Notification Delivery workflow (PR4 workflow 14) to drive a user-facing notification. This is a *consumption role*, not a separate wire format — an event already published as a Domain Event may also be consumed by the notification pipeline. This classification reflects Workflow 14's own trigger list exactly (`04-application-workflows.md` §14: "consumes `RecommendationGenerated`, `DecisionRecorded`, `ActionCompleted`, `OutcomeRecorded`, and equivalent integration events") — it is narrower than, and must not be confused with, PR4's raw per-event "Consumers" column in `04-command-query-event-catalog.md` §7, which additionally names "Notification" as a consumer for several other events (e.g. `WorkspaceCreated`, `PMOCreated`, `RecommendationRejected`, `AgentRunFailed`) through simpler, non-workflow notification paths not modeled as a distinct API contract here.

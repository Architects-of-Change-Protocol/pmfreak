...
| AddProjectStakeholder | Add a stakeholder record | Authorized Project member | Stakeholder record (via Project) | Project exists |
...
| GenerateRecommendation | Produce a Recommendation from an Agent Proposal | Agent (via Agent Orchestration) | Recommendation | Agent Proposal exists and passed validation |
...
| GenerateRecommendation | Agent Orchestration identity; AgentExecutionPolicy | agent run id | Recommendation creation | RecommendationGenerated | AgentExecutionError, PolicyViolation | No — generation is not a domain mutation of authoritative state |
...
Restating `04-canonical-application-architecture.md` §21: `ProjectCreated`, `EvidenceSubmitted`, `MemoryRecordApproved`, `RecommendationGenerated`, `DecisionRecorded`, `DecisionRevoked`, `ActionCompleted`, `OutcomeRecorded`, `EnterpriseKnowledgeRatified`, `EnterpriseKnowledgeRevoked`, `WorkspaceCreated`, `PMOCreated`, `PortfolioCreated`, `ProgramCreated` are the events most likely to require a versioned integration contract, since each already has a consumer in a different bounded context. Every other event in §7 may remain a purely internal domain event until a second-context consumer is actually designed.

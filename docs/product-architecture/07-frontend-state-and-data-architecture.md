...
Server state (§2) is cached per-Query, keyed by the Query's name and its resolved parameters (including the resolved tenant scope, never keyed in a way that could return one Workspace's cached result for another's request — restating ADR-PMF-061 at the cache-key layer). A Command's success invalidates exactly the Query cache entries that Command's documented Side Effects and Emitted Events (`06-command-catalog.md`'s per-Command columns) imply are affected — never a blanket cache-clear "to be safe," which would defeat the purpose of caching. `06-command-catalog.md` does not yet carry a dedicated affected-Query/invalidation field of its own (it documents Side Effects and Emitted Events, from which the affected Queries must currently be derived) — until that field exists, deriving the invalidation set from Side Effects/Emitted Events is the interim, still-precise rule; adding an explicit per-Command invalidation-target field to `06-command-catalog.md` is a named future contract addition (§13 of the parent document), not something the frontend silently guesses around in the meantime.

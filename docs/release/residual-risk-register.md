...
| -- | ---- | -------: | ---------: | -----: | ---------- | ----- | ----------- | ------------- |
| RR-MIGRATE | Perilla 13: fresh-apply proven against a *local* PostgreSQL 16 (144 migrations, 0 failures after remediation; 26 real migration defects found and fixed, including a blocking `workspace_memberships` RLS recursion bug — see `docs/release/migration-failure-remediation-log.md`) but **not yet against a hosted Supabase project or the official `supabase start` local stack** (no credentials / no Docker in this environment — honesty statement in `docs/release/fresh-database-migration-proof.md`) | M | L | H | Run `npm run check:fresh-db-migrations` (new, this PR) in hosted mode against an isolated Supabase project — needs `SUPABASE_PROJECT_REF` + `SUPABASE_ACCESS_TOKEN` + `SUPABASE_DB_URL`, **or** Docker access for the official local stack — then re-run `scripts/fresh-db-rls-smoke-test.sql` and the full E.3 role matrix against that environment | Repo owner | Before pilot start | **Yes** |

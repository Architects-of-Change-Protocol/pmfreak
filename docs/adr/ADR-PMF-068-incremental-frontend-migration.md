...
3. Migration units are prioritized by evidenced risk and value, and execution risk is prioritized ahead of decision risk: routes already classified "direct match" (`07-route-layout-and-navigation-architecture.md` §9) — the highest-confidence, lowest-ambiguity units — establish and prove the contract-client/module-boundary pattern first (`07-frontend-migration-strategy.md` §7 Phases 0–1), *before* the plan spends effort on routes with no canonical screen counterpart, because each of those requires its own per-route product decision (map onto an existing canonical screen, or deprecate — §9's classification, §7 Phase 4) that the direct-match routes do not, and resolving that decision-risk category first would block the pattern from ever being established and verified. "Resolved before reorganized," restated precisely: no no-canonical-counterpart route is left permanently undecided — every one is mapped or deprecated by the end of migration (`07-frontend-migration-strategy.md` §7 Phase 4, §16 Removal Criteria) — but it is decided after the pattern direct-match routes prove is available to apply to it, not before.

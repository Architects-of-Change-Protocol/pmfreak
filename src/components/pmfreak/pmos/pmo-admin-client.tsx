  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm text-slate-400">{pmos.length} PMO{pmos.length === 1 ? "" : "s"} in this workspace</p>
        <button
          type="button"
          onClick={openCreate}
      ...
      {editorOpen ? (
        <div className="space-y-4 rounded-2xl border border-white/15 bg-black/30 p-5">
          <h3 className="text-base font-semibold text-white">{creating ? "Create PMO" : "Edit PMO"}</h3>
              ...
              <div className="flex flex-wrap gap-1.5">
                {ICON_OPTIONS.map((icon) => (
                  <button
                    ...
                    className={`rounded-lg border px-2 py-1 text-base ${editor.icon === icon ? "border-cyan-300/60 bg-cyan-400/10" : "border-white/10 bg-white/[0.02] hover:border-white/25"}`}
                  >
                    {icon}
                  </button>
                ))}
              ...
              <div className="flex flex-wrap gap-1.5">
                {COLOR_OPTIONS.map((color) => (
                  <button
                    ...
                    className={`h-7 w-7 rounded-full border-2 ${editor.color === color ? "border-white" : "border-transparent"}`}
          ...
          <div className="flex gap-2">
            <button
              ...
              className="rounded-xl border border-cyan-200/45 bg-cyan-400/[0.1] px-4 py-2 text-sm font-semibold text-cyan-100 transition hover:bg-cyan-400/[0.16] disabled:opacity-50"
            >
      ...
      <div className="grid gap-3 md:grid-cols-2">
        {pmos.length === 0 ? (
          <div className="rounded-2xl border border-white/10 bg-black/20 p-5 text-sm text-slate-300 md:col-span-2">
            No PMOs yet. A PMO groups and governs a set of projects — create one to get started.
          </div>
        ) : (
          pmos.map((pmo) => (
            <article key={pmo.id} className="rounded-2xl border border-white/10 bg-black/25 p-4" style={{ borderLeftColor: pmo.color ?? undefined, borderLeftWidth: pmo.color ? 3 : undefined }}>
              <div className="flex items-start justify-between gap-3">
                <Link href={`/pmos/${pmo.id}`} className="group min-w-0">
                  <p className="truncate text-lg font-semibold text-cyan-100 group-hover:text-cyan-50">
              ...
              {pmo.description ? <p className="mt-2 line-clamp-2 text-sm text-zinc-300">{pmo.description}</p> : null}
              <div className="mt-3 flex flex-wrap gap-2 text-xs">
                <Link href={`/pmos/${pmo.id}`} className="rounded-lg border border-white/15 px-2.5 py-1.5 text-slate-200 hover:border-cyan-300/40">Open</Link>
                <Link href={`/pmos/${pmo.id}/chat`} className="rounded-lg border border-white/15 px-2.5 py-1.5 text-slate-200 hover:border-cyan-300/40">Chat</Link>
                <button type="button" disabled={busy} onClick={() => openEdit(pmo)} className="rounded-lg border border-white/15 px-2.5 py-1.5 text-slate-200 hover:border-cyan-300/40 disabled:opacity-50">Edit</button>
                <button
                  type="button"
                  disabled={busy}
                  onClick={() => void callApi(`/api/pmos/${pmo.id}/duplicate`, { method: "POST" })}
                  className="rounded-lg border border-white/15 px-2.5 py-1.5 text-slate-200 hover:border-cyan-300/40 disabled:opacity-50"
                >
                  Duplicate
                </button>
                <button
                  ...
                  className="rounded-lg border border-white/15 px-2.5 py-1.5 text-slate-200 hover:border-amber-300/40 disabled:opacity-50"
                >
                ...
                <button
                  ...
                  className="rounded-lg border border-rose-300/25 px-2.5 py-1.5 text-rose-200 hover:border-rose-300/50 disabled:opacity-50"
                >
                  Delete
                </button>
              </div>

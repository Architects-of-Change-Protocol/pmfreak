...
const ASSIGNMENT_TYPE_COLORS: Record<string, string> = {
  primary: "bg-indigo-500/20 text-indigo-700 border-indigo-500/30",
  secondary: "bg-cyan-500/20 text-cyan-700 border-cyan-500/30",
  program: "bg-violet-500/20 text-violet-700 border-violet-500/30",
  observer: "bg-zinc-500/20 text-zinc-700 border-zinc-500/30",
};
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-50 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-[#0c0c10] p-6 shadow-2xl">
        <h2 className="text-lg font-semibold text-slate-900">Assign Project Manager</h2>
        <form onSubmit={handleSubmit} className="mt-4 space-y-4">
          <div>
            <label className="block text-xs text-zinc-600 mb-1">Project Manager</label>
            {loadingPMs ? (
          ...
          <div>
            <label className="block text-xs text-zinc-600 mb-1">Assignment Type</label>
            <select
          ...
          <div className="flex gap-2 pt-2">
            <button type="button" onClick={onClose} className="flex-1 rounded-xl border border-slate-200 px-4 py-2 text-sm text-zinc-700 hover:bg-white">
              Cancel
            </button>
            <button type="submit" disabled={loading || !pmId} className="flex-1 rounded-xl border border-indigo-300/40 bg-indigo-400/10 px-4 py-2 text-sm font-semibold text-indigo-900 hover:bg-indigo-400/20 disabled:opacity-50">
    ...
    <>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-base font-semibold text-slate-900">Project Ownership</h2>
          <button
            onClick={() => setShowAssign(true)}
            className="rounded-xl border border-indigo-300/40 bg-indigo-400/10 px-3 py-1.5 text-xs font-semibold text-indigo-900 hover:bg-indigo-400/20"
          >
            Assign PM
          </button>
        </div>
        ...
        {!loading && assignments.length === 0 && (
          <p className="text-sm text-zinc-600">No PMs assigned to this project.</p>
        )}
              ...
              return (
                <div key={type}>
                  ...
                  <ul className="space-y-1">
                    {group.map((a) => (
                      <li key={a.id} className="flex items-center justify-between rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm font-medium text-slate-900">{a.pm_display_name ?? a.pm_id.slice(0, 8) + "…"}</span>
                          {a.pm_email && <span className="text-xs text-zinc-600">{a.pm_email}</span>}

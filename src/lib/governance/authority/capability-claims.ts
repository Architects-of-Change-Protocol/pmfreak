// PMFreak capability-claim issuance, hashing and verification.
// OWNERSHIP: PMFreak. Not Soberania Protocol, not Frontera.
// Upstream ships CAPABILITY_CLAIM_VERSION but no signing surface, so there is no
// canonical operation to adopt here. What IS canonical is the serialization
// profile: canonicalizeJSON comes from the packaged @aoc/protocol artifact, which
// is what keeps signatures over already-stored claims byte-identical.
// Claim version strings, the default trust domain and the accepted legacy values
// below are persisted and must not change.
// Trust resolution, revocation lookup, signing material and audit emission are
// injected through the PMFreak-owned ports in ./ports.
...
// issuer.app is typed as string so the claim format stays application-neutral.

...
export async function generateAndPersistOperationalGovernanceBrief({
  ...
  const [{ data: governanceRow }, { data: runtimeRow }, { data: raidRows }] = await Promise.all([
    ...
    supabase
      .from("raid_items")
      .select("id, workspace_id, project_id, source_document_id, source_signal_id, category, title, description, status, confidence_score, detected_at, detected_by, owner, due_date, auto_generated, fingerprint, occurrence_count")
      .eq("workspace_id", workspaceId)

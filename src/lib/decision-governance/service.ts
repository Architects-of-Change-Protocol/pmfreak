...
const columns = "id,workspace_id,project_id,decision_type,decision_status,title,summary,decision_rationale,recommendation_id,approved_by,rejected_by,created_by,created_at,approved_at,rejected_at,closed_at,metadata";
const evidenceColumns = "id,decision_id,evidence_id,evidence_type,relationship_type,created_at";
const outcomeColumns = "id,decision_id,outcome_reference_id,outcome_type,created_at";
...
async function emitDecisionEvent(decision: DecisionRecord, eventType: DecisionLifecycleEvent, actorId: string, correlationId?: string | null, causationId?: string | null): Promise<Result<DecisionRecord>> {
...
export const implementDecision = (decisionId: string, implementedBy: string, correlationId?: string | null, causationId?: string | null) => transitionDecision(decisionId, implementedBy, "implemented", "DECISION_IMPLEMENTED", { closed_at: new Date().toISOString() } as Partial<DecisionRecord>, correlationId, causationId);
...
export async function listProjectDecisions(projectId: string): Promise<Result<DecisionSummary[]>> {
  ...
  const supabase = await createSupabaseServerClient();
  const { data, error } = await supabase.from("project_decisions").select(`${columns}, decision_evidence_links(id), decision_outcome_links(id)`).eq("project_id", projectId).order("created_at", { ascending: false });
  if (error || !data) return { ok: false, error: "Unable to list decisions.", failureClass: "persistence_failed" };
  return { ok: true, data: (data as Array<DecisionRecord & { decision_evidence_links?: unknown[]; decision_outcome_links?: unknown[] }>).map((d) => ({ id: d.id, workspace_id: d.workspace_id, project_id: d.project_id, decision_type: d.decision_type, decision_status: d.decision_status, title: d.title, summary: d.summary, created_at: d.created_at, approved_at: d.approved_at, closed_at: d.closed_at, evidenceCount: d.decision_evidence_links?.length ?? 0, outcomeCount: d.decision_outcome_links?.length ?? 0 })) };
}
...
export async function buildDecisionLineage(decisionId: string): Promise<Result<DecisionLineage>> {
  ...
  const [{ data: evidence }, { data: outcomes }, { data: recommendation }] = await Promise.all([
    ...
    supabase.from("decision_outcome_links").select(outcomeColumns).eq("decision_id", decisionId).returns<DecisionOutcomeLink[]>(),
  ...
  return { ok: true, data: { decision, evidence: evidence ?? [], recommendations: recommendation ?? [], approvals: { approvedBy: decision.approved_by, approvedAt: decision.approved_at, rejectedBy: decision.rejected_by, rejectedAt: decision.rejected_at }, outcomes: outcomes ?? [], platformEvents: events.ok ? events.events : [] } };
}
...
export async function exportDecisionAuditPackage(decisionId: string): Promise<Result<DecisionAuditPackage>> {
  ...
  return { ok: true, data: { decision: lineage.data.decision, evidence: lineage.data.evidence, approvals: lineage.data.approvals, outcomes: lineage.data.outcomes, lineage: lineage.data, events: lineage.data.platformEvents } };
}

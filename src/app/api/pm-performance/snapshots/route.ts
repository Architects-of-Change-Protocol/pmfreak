...
import { generateWorkspacePMPerformanceSnapshots } from "@/lib/pm-performance";

const ROUTE = "/api/pm-performance/snapshots";
...
export async function POST() {
  try {
    ...
    const result = await generateWorkspacePMPerformanceSnapshots({ workspaceId, actorId: user.id });
    if (!result.ok) {

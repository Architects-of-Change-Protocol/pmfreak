...
import { archiveProgramCard, updateProgramCard } from "@/lib/program-cards";
import type { ProgramItemStatus } from "@/lib/program-cards";

const ROUTE = "/api/programs/[id]/cards/[cardId]";
...
export async function PATCH(
  ...
  try {
    ...
    const descriptionField = "description" in body
      ...
      : undefined;
    const promptBodyField = "promptBody" in body
      ? (body.promptBody === null || typeof body.promptBody === "string" ? body.promptBody as string | null : (() => { throw new Error("invalid_promptBody"); })())
      : undefined;
    ...
    const result = await updateProgramCard(cardId, workspaceId, {
      title: titleField,
      description: descriptionField,
      promptBody: promptBodyField,
      status: statusField,
...
export async function DELETE(
  ...
  try {
    ...
    await requireWorkspaceMember(workspaceId);
    const result = await archiveProgramCard(cardId, workspaceId, user.id);
    if (!result.ok) {

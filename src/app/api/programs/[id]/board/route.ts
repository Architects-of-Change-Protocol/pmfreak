...
import { getProgramExecutionBoard } from "@/lib/program-board";

const ROUTE = "/api/programs/[id]/board";
...
export async function GET(
  ...
  try {
    ...
    const result = await getProgramExecutionBoard({ workspaceId, programId: id });
    ...
    const { backlog, ready, inProgress, inReview, done, stats } = result.data;
    return NextResponse.json({ backlog, ready, inProgress, inReview, done, stats });
  } catch (error) {

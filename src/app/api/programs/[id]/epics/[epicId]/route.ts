...
import { archiveProgramEpic, updateProgramEpic } from "@/lib/program-epics";
import type { ProgramItemStatus } from "@/lib/program-epics";

const ROUTE = "/api/programs/[id]/epics/[epicId]";
...
export async function PATCH(
  ...
  try {
    ...
    const result = await updateProgramEpic(epicId, workspaceId, {
      title: titleField,
    ...
    return NextResponse.json({ epic: result.data });
  } catch (error) {
...
export async function DELETE(
  ...
  try {
    ...
    await requireWorkspaceMember(workspaceId);
    const result = await archiveProgramEpic(epicId, workspaceId, user.id);
    if (!result.ok) {

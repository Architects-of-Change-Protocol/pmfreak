...
import { parseProgramRoadmapSource } from "@/lib/program-roadmap-parser";

const ROUTE = "/api/programs/[id]/roadmap-sources/[sourceId]/parse";
...
export async function POST(
  ...
  try {
    ...
    const result = await parseProgramRoadmapSource({
      workspaceId,
      programId: id,
      sourceId,
      actorId: user.id,
    ...
    return NextResponse.json({ parseResult: result.data.row, parsed: result.data.parseResult }, { status: 201 });
  } catch (error) {

...
import { approveDecision, getDecision } from "@/lib/decision-governance/service";

const ROUTE_ID = "/api/decisions/[id]/approve";

/** ApproveRecommendation / RecordDecision approval step (06-command-catalog.md) — governance-relevant, never optimistic. */
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const result = await approveDecision(decisionId, userId!, body.rationale ?? null);

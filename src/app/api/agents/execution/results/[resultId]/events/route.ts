...
import { listAgentExecutionResultEvents } from "@/lib/agents";

const ROUTE = "/api/agents/execution/results/[resultId]/events";
...
export async function GET(request: Request, { params }: { params: Promise<{ resultId: string }> }) {
  try {
    ...
    const events = await listAgentExecutionResultEvents({
      workspaceId,
      resultId,

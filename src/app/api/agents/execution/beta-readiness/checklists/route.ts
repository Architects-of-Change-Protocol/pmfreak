...
import { runBetaOnboardingChecklist, listBetaOnboardingChecklists } from "@/lib/agents";
...
export async function POST(request: Request) {
  try {
    ...
    const record = await runBetaOnboardingChecklist(body.workspaceId, body.planId);
    return NextResponse.json({ ok: true, data: record }, { status: 201 });
  } catch (err) {
    return safeInternalErrorResponse("/api/agents/execution/beta-readiness/checklists", err);
  }
}
...
export async function GET(request: Request) {
  try {
    ...
    const records = await listBetaOnboardingChecklists(workspaceId, planId);

...
import { runTenantReadinessValidation, listTenantReadinessValidations } from "@/lib/agents";
...
export async function POST(request: Request) {
  try {
    ...
    const records = await runTenantReadinessValidation(body.workspaceId, body.planId);
    return NextResponse.json({ ok: true, data: records }, { status: 201 });
  } catch (err) {
    return safeInternalErrorResponse("/api/agents/execution/beta-readiness/tenant-validations", err);
  }
}
...
export async function GET(request: Request) {
  try {
    ...
    const records = await listTenantReadinessValidations(workspaceId, planId);

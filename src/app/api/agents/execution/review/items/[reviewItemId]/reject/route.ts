...
const ROUTE = "/api/agents/execution/review/items/[reviewItemId]/reject";
...
export async function POST(request: Request, { params }: { params: Promise<{ reviewItemId: string }> }) {
  try {
    ...
    const decision = await recordReviewDecision({ workspaceId, reviewItemId, decisionType: "reject", decidedBy: user.id, rationale: body.rationale, payload: body.payload });

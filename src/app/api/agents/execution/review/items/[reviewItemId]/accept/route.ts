...
const ROUTE = "/api/agents/execution/review/items/[reviewItemId]/accept";
...
export async function POST(request: Request, { params }: { params: Promise<{ reviewItemId: string }> }) {
  try {
    ...
    const decision = await recordReviewDecision({ workspaceId, reviewItemId, decisionType: "accept", decidedBy: user.id, rationale: body.rationale, payload: body.payload });

...
import { createProjectHandoffGate, listAgentPmoProjectHandoffGates } from "@/lib/agents";
...
export async function POST(request: Request) {
  try {
    ...
    const record = await createProjectHandoffGate(body);
    return NextResponse.json({ ok: true, data: record }, { status: 201 });
  } catch (err) {
    return safeInternalErrorResponse("/api/agents/execution/project-handoff/gates", err);
  }
}
...
export async function GET(request: Request) {
  try {
    ...
    const records = await listAgentPmoProjectHandoffGates(workspaceId, handoffRequestId);

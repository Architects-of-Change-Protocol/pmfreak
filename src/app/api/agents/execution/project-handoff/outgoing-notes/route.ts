...
import { recordOutgoingPmNote, listAgentPmoOutgoingPmNotes } from "@/lib/agents";
...
export async function POST(request: Request) {
  try {
    ...
    const record = await recordOutgoingPmNote(body);
    return NextResponse.json({ ok: true, data: record }, { status: 201 });
  } catch (err) {
    return safeInternalErrorResponse("/api/agents/execution/project-handoff/outgoing-notes", err);
  }
}
...
export async function GET(request: Request) {
  try {
    ...
    const records = await listAgentPmoOutgoingPmNotes(workspaceId, handoffRequestId);

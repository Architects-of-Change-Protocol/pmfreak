...
import { getAgentPmoProjectHandoffPackById, updateAgentPmoProjectHandoffPackStatus, validateAgentPmoProjectHandoffPackStatus } from "@/lib/agents";
...
export async function POST(request: Request, { params }: { params: Promise<{ handoffPackId: string }> }) {
  try {
    ...
    const existing = await getAgentPmoProjectHandoffPackById(handoffPackId);

...
export async function GET(request: Request, { params }: { params: Promise<{ exportId: string }> }) {
  try {
    ...
    const record = await getAgentPmoRuntimeHardeningExportById(exportId);
    ...
    const contentType = record.exportFormat === "json" ? "application/json" : record.exportFormat === "csv" ? "text/csv" : "text/markdown";
    return new Response(record.safeExportContent, {
      headers: {
        "Content-Type": contentType,
        "Content-Disposition": `attachment; filename="hardening-export-${exportId}.${record.exportFormat}"`,
      },
    });
  } catch (err) {

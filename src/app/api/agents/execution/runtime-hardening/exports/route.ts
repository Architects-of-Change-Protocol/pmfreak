...
import { generateRuntimeHardeningExport, listAgentPmoRuntimeHardeningExports } from "@/lib/agents";
...
export async function POST(request: Request) {
  try {
    ...
    const record = await generateRuntimeHardeningExport(body.workspaceId, body.hardeningRunId, body.format ?? "markdown");
    return NextResponse.json({ ok: true, data: record }, { status: 201 });
  } catch (err) {
    return safeInternalErrorResponse("/api/agents/execution/runtime-hardening/exports", err);
  }
}
...
export async function GET(request: Request) {
  try {
    ...
    const records = await listAgentPmoRuntimeHardeningExports(workspaceId, hardeningRunId);

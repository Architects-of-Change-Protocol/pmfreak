...
import { runExportSafetyCheck, listAgentPmoExportSafetyChecks } from "@/lib/agents";
...
export async function POST(request: Request) {
  try {
    ...
    const record = await runExportSafetyCheck(body.workspaceId, body.hardeningRunId);
    return NextResponse.json({ ok: true, data: record }, { status: 201 });
  } catch (err) {
    return safeInternalErrorResponse("/api/agents/execution/runtime-hardening/export-safety", err);
  }
}
...
export async function GET(request: Request) {
  try {
    ...
    const records = await listAgentPmoExportSafetyChecks(workspaceId, hardeningRunId);

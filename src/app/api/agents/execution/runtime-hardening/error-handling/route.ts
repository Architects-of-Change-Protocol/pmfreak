...
import { runErrorHandlingCheck, listAgentPmoErrorHandlingChecks } from "@/lib/agents";
...
export async function POST(request: Request) {
  try {
    ...
    const record = await runErrorHandlingCheck(body.workspaceId, body.hardeningRunId);
    return NextResponse.json({ ok: true, data: record }, { status: 201 });
  } catch (err) {
    return safeInternalErrorResponse("/api/agents/execution/runtime-hardening/error-handling", err);
  }
}
...
export async function GET(request: Request) {
  try {
    ...
    const records = await listAgentPmoErrorHandlingChecks(workspaceId, hardeningRunId);

...
import { runIdempotencyGuardCheck, listAgentPmoIdempotencyChecks } from "@/lib/agents";
...
export async function POST(request: Request) {
  try {
    ...
    const record = await runIdempotencyGuardCheck(body.workspaceId, body.hardeningRunId);
    return NextResponse.json({ ok: true, data: record }, { status: 201 });
  } catch (err) {
    return safeInternalErrorResponse("/api/agents/execution/runtime-hardening/idempotency", err);
  }
}
...
export async function GET(request: Request) {
  try {
    ...
    const records = await listAgentPmoIdempotencyChecks(workspaceId, hardeningRunId);

...
export async function GET(request: Request, { params }: { params: { planningWorkspaceId: string } }) {
  try {
    ...
    const workspace = await getAgentPmoImplementationPlanningWorkspaceById(workspaceId, params.planningWorkspaceId);
    if (!workspace) {

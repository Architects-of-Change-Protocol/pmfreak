...
export async function POST(request: Request, { params }: { params: { changeWindowId: string } }) {
  try {
    ...
    const updated = await updateAgentPmoChangeWindowStatus(workspaceId, params.changeWindowId, status);
    if (!updated) {

...
export async function POST(request: Request, { params }: { params: { prerequisiteId: string } }) {
  try {
    ...
    const updated = await updateAgentPmoImplementationGatePrerequisiteStatus(workspaceId, params.prerequisiteId, status, rationale ?? null);
    if (!updated) {

...
export async function POST(request: Request, { params }: { params: { planDraftId: string } }) {
  try {
    ...
    const updated = await updateAgentPmoImplementationPlanDraftStatus(workspaceId, params.planDraftId, status);
    if (!updated) {

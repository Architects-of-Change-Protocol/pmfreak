// POST /api/agents/memory/[memoryId]/expire
...
import { expireMemory } from "@/lib/agents";

const ROUTE = "/api/agents/memory/[memoryId]/expire";
...
export async function POST(request: Request, { params }: { params: Promise<{ memoryId: string }> }) {
  try {
    ...
    const memory = await expireMemory({ workspaceId, memoryId, actorId: user.id, reason: body?.reason ?? null });
    ...
    return NextResponse.json({ ok: false, error: { code: "INTERNAL", message: "Failed to expire memory." } }, { status: 500 });
  }
}

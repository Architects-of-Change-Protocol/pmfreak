...
import { markMemoryStale } from "@/lib/agents";

const ROUTE = "/api/agents/memory/[memoryId]/stale";
...
export async function POST(request: Request, { params }: { params: Promise<{ memoryId: string }> }) {
  try {
    ...
    const memory = await markMemoryStale({ workspaceId, memoryId, actorId: user.id, reason: body?.reason ?? null });
    ...
    return NextResponse.json({ ok: false, error: { code: "INTERNAL", message: "Failed to mark memory stale." } }, { status: 500 });
  }
}

...
const ROUTE = "GET /api/pmo-command-center";
...
export async function GET(_request: NextRequest) {
  try {
    ...
    const result = await getPMOCommandCenter({ workspaceId, actorId: user.id });

    if (!result.ok) {
      const statusMap: Record<string, number> = {
        PMO_COMMAND_CENTER_WORKSPACE_REQUIRED: 403,
        PMO_COMMAND_CENTER_UNAUTHORIZED: 403,
        PMO_COMMAND_CENTER_FAILED: 500,
      };

...
export default async function WorkspaceChatPage() {
  ...
  if (!resolution.workspaceId) {
    return <main className="rounded-3xl border border-white/10 bg-white/5 p-8 text-sm text-slate-300">No active workspace. Create one to start.</main>;
  }
  ...
  return (
    <main className="space-y-5">
      <header className="rounded-3xl border border-white/10 bg-white/[0.03] p-6">
        ...
        <h1 className="mt-2 text-3xl font-semibold tracking-tight text-white">{workspace?.name ?? "Workspace"} — Chat</h1>
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-3">
          <div className="rounded-2xl border border-white/10 bg-black/25 p-4">
            ...
            <p className="mt-1 text-lg font-semibold text-cyan-200">{pmos.length}</p>
          </div>
          <div className="rounded-2xl border border-white/10 bg-black/25 p-4">
            <p className="text-[11px] uppercase tracking-[0.14em] text-zinc-500">Projects</p>
            <p className="mt-1 text-lg font-semibold text-cyan-200">{totalProjects}</p>
          </div>
          <div className="rounded-2xl border border-white/10 bg-black/25 p-4">
            <p className="text-[11px] uppercase tracking-[0.14em] text-zinc-500">Active</p>
            <p className="mt-1 text-lg font-semibold text-emerald-200">{activeProjects}</p>
          </div>
        </div>
        {pmos.length === 0 ? (
          <p className="mt-4 text-sm text-slate-400">
            No PMOs yet. <Link href="/pmos" className="text-cyan-200 underline-offset-2 hover:underline">Create your first PMO</Link> to organize projects.
          </p>
        ) : null}
      </header>

  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-50 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-[#0c0c10] p-6 shadow-2xl">
        <h2 className="text-lg font-semibold text-slate-900">Register Project Manager</h2>
        <form onSubmit={handleSubmit} className="mt-4 space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 placeholder:text-zinc-500 focus:outline-none focus:ring-1 focus:ring-indigo-400"
            />
          </div>
          <div>
          ...
          <div className="flex gap-2 pt-2">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 rounded-xl border border-slate-200 px-4 py-2 text-sm text-zinc-700 hover:bg-white"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={loading}
              className="flex-1 rounded-xl border border-indigo-300/40 bg-indigo-400/10 px-4 py-2 text-sm font-semibold text-indigo-900 hover:bg-indigo-400/20 disabled:opacity-50"
            >
    ...
    <>
      ...
      <section className="relative overflow-hidden rounded-3xl border border-slate-200 bg-[#050507] p-6 shadow-[0_40px_120px_rgba(0,0,0,0.55)] md:p-10">
        ...
        <div className="relative space-y-8">
          <header className="relative overflow-hidden rounded-3xl border border-slate-200 bg-slate-50 p-6 md:p-8">
            ...
            <div className="relative max-w-3xl">
              <p className="text-xs uppercase tracking-[0.24em] text-indigo-800">PM Registry</p>
              <h1 className="mt-3 text-3xl font-semibold tracking-tight text-slate-900 md:text-4xl">Project Manager Registry</h1>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <h2 className="mt-1 text-xl font-semibold text-slate-900">Registry</h2>
            </div>
            <button
              onClick={() => setShowCreate(true)}
              className="rounded-xl border border-indigo-300/40 bg-indigo-400/[0.1] px-4 py-2 text-sm font-semibold text-indigo-900 transition hover:bg-indigo-400/[0.18]"
            >
              Register PM
            </button>
          </div>
          ...
          {loading && (
            <div className="rounded-2xl border border-slate-200 bg-white p-8 text-center">
              <p className="text-sm text-zinc-600">Loading project managers…</p>
            </div>
          )}
          ...
          {!loading && error && (
            <div className="rounded-2xl border border-red-500/20 bg-red-500/10 p-6">
              ...
              <button onClick={fetchPMs} className="mt-2 text-xs text-red-700 underline">Retry</button>
            </div>
          )}

          {!loading && !error && pms.length === 0 && (
            <div className="rounded-2xl border border-slate-200 bg-white p-10 text-center">
              <p className="text-sm text-zinc-600">No project managers registered yet.</p>
              <button
                onClick={() => setShowCreate(true)}
                className="mt-4 rounded-xl border border-indigo-300/40 bg-indigo-400/10 px-4 py-2 text-sm font-semibold text-indigo-900 hover:bg-indigo-400/20"
              >
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-200 bg-white text-left">
                    <th className="px-4 py-3 text-xs font-medium uppercase tracking-wider text-zinc-600">Name</th>
                    <th className="px-4 py-3 text-xs font-medium uppercase tracking-wider text-zinc-600">Email</th>
                    <th className="px-4 py-3 text-xs font-medium uppercase tracking-wider text-zinc-600">Status</th>
                    <th className="px-4 py-3 text-xs font-medium uppercase tracking-wider text-zinc-600">Registered</th>
                ...
                <tbody>
                  {pms.map((pm, i) => (
                    <tr
                      key={pm.id}
                      className={`border-b border-slate-200 hover:bg-white transition-colors ${i % 2 === 0 ? "bg-transparent" : "bg-white"}`}
                    >
                      <td className="px-4 py-3 font-medium text-slate-900">{pm.display_name}</td>
                      <td className="px-4 py-3 text-zinc-700">{pm.email}</td>
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-medium ${STATUS_COLORS[pm.status] ?? ""}`}>
                          {pm.status}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-zinc-600">{new Date(pm.created_at).toLocaleDateString()}</td>

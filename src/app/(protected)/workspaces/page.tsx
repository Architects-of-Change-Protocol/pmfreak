  ...
  return (
    <main className="space-y-5">
      <header className="rounded-3xl border border-white/10 bg-white/[0.03] p-6">
        <p className="text-xs uppercase tracking-[0.24em] text-cyan-200">Account</p>
      ...
      <section className="grid gap-3 md:grid-cols-2">
        {workspaces.map((workspace) => {
          ...
          return (
            <article key={workspace.id} className={`rounded-2xl border p-4 ${isActive ? "border-cyan-300/40 bg-cyan-400/[0.06]" : "border-white/10 bg-black/25"}`}>
              <div className="flex items-center justify-between gap-3">
                <div>
                  <h2 className="text-lg font-semibold text-white">{workspace.name}</h2>
                  {isActive ? <p className="text-[11px] uppercase tracking-[0.14em] text-cyan-200">Active workspace</p> : null}
                </div>
                {!isActive ? (
                  <form action={switchWorkspaceAction}>
                    <input type="hidden" name="workspaceId" value={workspace.id} />
                    <button type="submit" className="rounded-xl border border-white/15 px-3.5 py-2 text-sm text-slate-200 hover:border-cyan-300/40">
                      Switch
                    </button>
                  </form>
                ) : (
                  <Link href="/pmos" className="rounded-xl border border-white/15 px-3.5 py-2 text-sm text-slate-200 hover:border-cyan-300/40">
                    Open PMOs
                  </Link>
                )}
              </div>
